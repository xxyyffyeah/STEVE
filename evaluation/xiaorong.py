...
def config():
    ...
    parser.add_argument("--shots", type=int, default=-1, help="Number of few-shot demonstrations to append to the starting system prompt (0 for zero-shot).")
...
if args.shots > 0 and len(train_set_list) > 0:
    chosen_examples = random.sample(train_set_list, min(args.shots, len(train_set_list)))
    few_shot_text = _build_few_shot_block(chosen_examples)
    if few_shot_text:
    ...
    print(f"Using {len(chosen_examples)} few-shot examples appended to the system prompt.")
